...
class OrgParserTest : StringSpec ({
    ...
    "testParse_TokenMapping should preserve all tokens in the right order" {
        val document = parse(tokens)

        unparse(document!!) shouldBe tokens
    }

    "testParse_TextRecovery should reconstruct the text back after a round trip" {
